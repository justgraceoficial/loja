...
(function(){
  ...
  async function requestOnlinePayment(order, method){
    ...
    let res;
    try{
      res = await fetch(SYNC_CONFIG.apiUrl, {
        method: 'POST',
        headers: { 'Content-Type': 'text/plain;charset=utf-8' },
        body: JSON.stringify({
    ...
    try{
      ...
      let bodyText = '';
      try{ bodyText = (await res.clone().text()).slice(0, 140); }catch(e2){}
      return { error: 'O servidor de pagamento respondeu de forma inesperada (status ' + res.status + ').' + (bodyText ? ' Trecho: ' + bodyText : '') };
    }
  }
