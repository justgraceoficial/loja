...
(function(){
  ...
  /* ----------------------------------------------------------------
     ...
     ---------------------------------------------------------------- */
  const SYNC_CONFIG = {
    apiUrl: 'https://script.google.com/macros/s/AKfycbxOSaSq5sm5Bv-_Fb73e5ti63KWdHWBWgNs0eZJYAThdgDK3sdESYMdFTrQ_nkbfyVtZw/exec',
  ...
  // O Safari lida mal com fetch() em requisições que passam pelo redirecionamento
  // interno do Google Apps Script — por isso usamos XMLHttpRequest aqui, que é
  // mais compatível nesse caso específico.
  function xhrRequest(method, url, bodyText){
    return new Promise((resolve, reject) => {
      ...
      xhr.send(bodyText !== undefined ? bodyText : null);
    });
  }
  ...
  async function pushListToServer(type, list){
    ...
    try{
      await xhrRequest('POST', SYNC_CONFIG.apiUrl, JSON.stringify({ token: SYNC_CONFIG.token, type, [type]: list }));
    }catch(err){
  ...
  async function pullListFromServer(type, key){
    ...
    try{
      const text = await xhrRequest('GET', SYNC_CONFIG.apiUrl + '?token=' + encodeURIComponent(SYNC_CONFIG.token) + '&type=' + type);
  ...
  async function requestOnlinePayment(order, method){
    ...
    try{
      ...
      return { error: 'O servidor de pagamento respondeu de forma inesperada.' + (text ? ' Trecho: ' + text.slice(0,140) : '') };
    }
  }
